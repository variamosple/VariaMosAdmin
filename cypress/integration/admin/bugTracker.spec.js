...
describe('Admin - Bug Tracker E2E Flows', () => {
  ...
  it('should display GitHub bugs and update list when filters are applied', () => {
    ...
    cy.get('.tab-pane.active table tbody tr').eq(0).should('contain', 'GitHub UI Crash');
    cy.get('.tab-pane.active table tbody tr').eq(1).should('contain', 'GitHub Performance lag');
    ...
    cy.get('.tab-pane.active table tbody tr').eq(0).should('contain', 'GitHub Performance lag');
    cy.get('.tab-pane.active table tbody tr').should('not.contain', 'GitHub UI Crash');
  ...
  it('should display Local Inbox bugs, access attachments, and perform Approve/Reject actions', () => {
    ...
    cy.intercept('POST', /\/bugs\/local-1\/status$/, {
      statusCode: 200,
      body: { transactionId: 'updateStatus', data: { id: 'local-1', status: 'open' } }
    }).as('approveBug');

    cy.intercept('POST', /\/bugs\/local-1\/reject$/, {
      statusCode: 200,
      body: { transactionId: 'rejectBug', data: { id: 'local-1', status: 'rejected' } }
    }).as('rejectBug');
    ...
    cy.get('.tab-pane.active table tbody tr').eq(0).within(() => {
      cy.get('a').should('have.attr', 'href').and('contain', '/uploads/screenshot.png');
    ...
    cy.get('.modal-footer').contains('button', 'Approve & Send to GitHub').click();
    cy.wait('@approveBug');
  ...
  it('should display Trash Bin bugs and perform Restore action', () => {
    ...
    cy.intercept('POST', /\/bugs\/local-2\/restore$/, {
      statusCode: 200,
      body: { transactionId: 'restoreBug', data: { id: 'local-2', status: 'pending' } }
    }).as('restoreBug');
  ...
  it('should handle User Mode Modal (UserBugFormModal) visibility, validation, and submission', () => {
    ...
    cy.wait('@createUserBug').then((interception) => {
      ...
      const bodyString = typeof body === 'string' ? body : new TextDecoder('utf-8').decode(body);
  ...
  it('should handle BugApprovalModal review workflow, editing fields, uploading/deleting attachments, and verifying status change POST payload', () => {
    ...
    cy.get('.modal textarea[name="description"]').should('have.value', 'Original Description');
    cy.get('.modal select[name="githubRepo"]').should('have.value', 'VariaMos/VariaMosPLE');
    ...
    cy.get('.modal .list-group-item').contains('old_screenshot.png').parent().find('button').click();
    ...
    cy.get('.modal textarea[name="description"]').clear().type('Revised Description');
    cy.get('.modal select[name="githubRepo"]').select('VariaMos/VariaMosAdmin');
    cy.get('.modal select[name="priority"]').select('high');
    cy.get('.modal select[name="category"]').select('Editor');
